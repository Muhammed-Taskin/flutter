...
class BurcListesi extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final burclar = Strings.BURC_ADLARI; // Listeyi Strings sınıfından aldık
    final tarihler = Strings.BURC_TARIHLERI;

    return Scaffold(
      backgroundColor: const Color.fromARGB(255, 18, 45, 202),//renklendirme
      appBar: AppBar(
        title: const Text(Strings.APP_NAME),
        backgroundColor: const Color.fromARGB(255, 10, 220, 199),
      ),
        ...
        itemBuilder: (context, index) {
          return Card(
            ...
            child: ListTile(
              ...
              subtitle: Text(tarihler[index]),
              trailing: const Icon(Icons.arrow_forward_ios, size: 16),
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => BurcDetaySayfasi(
                      burcAdi: burclar[index],
                      burcDetay: Strings.BURC_GENEL_OZELLIKLERI[index],
                    ),
                  ),
                );
              },
...
class BurcDetaySayfasi extends StatelessWidget {
  final String burcAdi;
  final String burcDetay;

  const BurcDetaySayfasi({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(burcAdi)),
      body: Padding(
...
class Strings {
  ...
  static const List<String> BURC_GENEL_OZELLIKLERI = [
    // Burç açıklamaları burada senin yazdığın gibi uzun metinler...
      "\tGenel Özellikler\n\n"
